...
const ProfileScreen = ({ location, history }: RouteComponentProps) => {
  ...
  const userUpdateProfile = useSelector((state: RootState) => state.userUpdate);
  const { success } = userUpdateProfile;

  const userOrderList = useSelector((state: RootState) => state.userOrderList);
  ...
  const submitHandler = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (password !== confirmPassword) {
      ...
      dispatch(
        updateUserProfile({ userInfo: { id: user._id, name, email, password } })
      );
    }
  };
